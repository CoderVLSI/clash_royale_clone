...
const CARDS = [
  ...
  { id: 'x_bow', name: 'X-Bow', cost: 6, color: '#95a5a6', hp: 700, speed: 0, type: 'building', range: 280, damage: 40, attackSpeed: 100, projectile: 'arrow', count: 1, lifetime: 30, targetType: 'buildings', rarity: 'epic', spawnDelay: 3500 },
        ...
        if (closestTarget && minDist <= actualRange + 25) {
          ...
          if (now - u.lastAttack > currentAttackSpeed && !isWakingUp) {
            ...
            if (u.projectile) {
              ...
              if (u.kamikaze) {
                // Heal Spirit - heal nearby friendly units on attack
                ...
                // Electro Spirit - chain damage to multiple targets
                if (u.chain > 0) {
                  chainEvents.push({
                    attackerId: u.id,
                    primaryTarget: closestTarget,
                    chainCount: u.chain - 1, // -1 because first target is already hit
                    damage: damageToDeal,
                    stun: u.stun || 0,
                    isOpponent: u.isOpponent
                  });
