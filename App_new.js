...
const MainMenu = ({
  ...
  const TIPS = ["Tip: Destroying enemy towers grants Crowns!", "Tip: Join a Clan to request cards and friendly battle!", "Tip: Don't spend all your Elixir at once!", "Tip: Lure enemy troops to the center to activate your King Tower.", "Tip: Use spells to damage multiple units at once.", "Tip: Balance your deck with ground and air units."];
  useEffect(() => {
  ...
  return <ImageBackground source={require('./background.jpg')} style={styles.menuContainer}>
...
const UnitSprite = ({
  ...
  switch (id) {
    ...
    case 'baby_dragon':
      const babyDragonSprite = <Svg width={size} height={size} viewBox="0 0 100 100">
          <Circle cx="50" cy="50" r="45" fill={color} stroke="white" strokeWidth="2" />
    ...
    case 'minions':
    case 'minion_horde':
      const minionSprite = <Svg width={size} height={size} viewBox="0 0 100 100">
    ...
    case 'tesla':
      ...
      return <Svg width={size} height={size} viewBox="0 0 100 100" opacity={opacity}>
    ...
    case 'lava_hound':
      const lavaHoundSprite = <Svg width={size} height={size} viewBox="0 0 100 100">
          {/* Outer glow */}
    ...
    case 'lava_pups':
      const lavaPupSprite = <Svg width={size} height={size} viewBox="0 0 100 100">
          {/* Glow */}
    ...
    default:
      const sprite = <Svg width={size} height={size} viewBox="0 0 100 100">
          <Circle cx="50" cy="50" r="45" fill={color} stroke="white" strokeWidth="2" />
...
const Card = memo(({
  ...
  const displayCost = card.id === 'mirror' && lastPlayedCard ? lastPlayedCard.cost + 1 : card.cost;
  ...
  return <View style={[styles.card, !isLegendary && {
    borderColor: RARITY_COLORS[cardToDisplay.rarity] || '#000'
  }, isLegendary && {
    backgroundColor: 'transparent',
    borderWidth: 0
  }, isNext && styles.nextCard, !canAfford && !isNext && styles.disabledCard, isDragging && styles.hiddenCard]} {...handlers}>
      {isLegendary && <Svg width={isNext ? "40" : "60"} height={isNext ? "50" : "75"} viewBox="0 0 60 75" style={{
      position: 'absolute',
      top: 0,
      left: 0
    }}>
          ...
          <Polygon points="30,2 58,18 58,57 30,73 2,57 2,18" fill="#ecf0f1" stroke="url(#rainbow)" strokeWidth="3" />
        </Svg>}
  ...
  return prevProps.canAfford === nextProps.canAfford && prevProps.isDragging === nextProps.isDragging && prevProps.card?.id === nextProps.card?.id;
});
const HealthBar = ({
  current,
  max,
  ...
  return <View style={{
    ...
    zIndex: 20
      ...
      {hasShield && shieldHp > 0 && <View style={{
      width: '100%',
      height: 6,
...
const VisualEffects = ({
  ...
  return <>
      ...
      if (effect.type === 'fire_explosion') {
        ...
        return <View key={effect.id} style={{
          position: 'absolute',
          ...
          justifyContent: 'center'
              ...
              <Svg width={effect.radius * 2} height={effect.radius * 2} viewBox={`0 0 ${effect.radius * 2} ${effect.radius * 2}`}>
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * (0.3 + progress * 0.7)} fill="#e74c3c" opacity={0.6} />
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * (0.5 + progress * 0.5)} fill="#f39c12" opacity={0.4} />
              </Svg>
            </View>;
      }
      if (effect.type === 'ice_freeze') {
        ...
        return <View key={effect.id} style={{
          ...
          justifyContent: 'center'
              ...
              <Svg width={effect.radius * 2} height={effect.radius * 2} viewBox={`0 0 ${effect.radius * 2} ${effect.radius * 2}`}>
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * (0.3 + progress * 0.7)} fill="#3498db" opacity={0.5} />
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * 0.5} fill="white" opacity={0.3} />
              </Svg>
      ...
      if (effect.type === 'heal_glow') {
        ...
        return <View key={effect.id} style={{
          ...
          justifyContent: 'center'
              ...
              <Svg width={effect.radius * 2} height={effect.radius * 2} viewBox={`0 0 ${effect.radius * 2} ${effect.radius * 2}`}>
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * (0.2 + progress * 0.8)} fill="#2ecc71" opacity={0.4} />
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * 0.6} fill="#27ae60" opacity={0.2} />
              </Svg>
        ...
        return <View key={effect.id} style={{
          ...
          justifyContent: 'center'
              ...
              <Svg width={effect.radius * 2} height={effect.radius * 2} viewBox={`0 0 ${effect.radius * 2} ${effect.radius * 2}`}>
                ...
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * (0.3 + progress * 0.7)} fill="none" stroke="#95a5a6" strokeWidth="3" strokeDasharray="10 5" opacity={0.8} />
                {/* Inner shatter effect */}
                <Path d={`M${effect.radius * 0.5} ${effect.radius * 0.5} L${effect.radius * 1.5} ${effect.radius * 1.5} M${effect.radius * 1.5} ${effect.radius * 0.5} L${effect.radius * 0.5} ${effect.radius * 1.5}`} stroke="#bdc3c7" strokeWidth="2" opacity={0.6} />
                {/* Shield icon breaking */}
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * 0.3} fill="#3498db" opacity={0.5 - progress * 0.5} />
              </Svg>
              {/* Shield break text */}
              <View style={{
            position: 'absolute',
      ...
      if (effect.type === 'chain_lightning' && effect.targets && effect.targets.length > 1) {
        ...
        for (let i = 0; i < effect.targets.length - 1; i++) {
          ...
          pathD += ` L${end.x} ${end.y}`;
          lines.push(<Svg key={`${effect.id}-${i}`} style={{
            position: 'absolute',
            left: 0,
            top: 0,
            width: '100%',
            ...
            opacity: opacity
                ...
                <Path d={pathD} stroke="#f1c40f" strokeWidth="3" fill="none" strokeLinecap="round" />
                <Path d={pathD} stroke="#3498db" strokeWidth="6" fill="none" opacity={0.5} strokeLinecap="round" />
              </Svg>);
        }
      ...
      if (effect.type === 'heal_pulse') {
        ...
        return <View key={effect.id} style={{
          ...
          justifyContent: 'center'
              ...
              <Svg width={effect.radius * 2} height={effect.radius * 2} viewBox={`0 0 ${effect.radius * 2} ${effect.radius * 2}`}>
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * (0.4 + progress * 0.6)} fill="rgba(241, 196, 15, 0.3)" stroke="#F1C40F" strokeWidth="2" opacity={0.5} />
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * 0.3} fill="#FFF176" opacity={0.2} />
              </Svg>
            </View>;
      }
        ...
        return <View key={effect.id} style={{
          ...
          zIndex: 50
              ...
              <Svg width={Math.abs(dx)} height={Math.abs(dy)} viewBox={`0 0 ${Math.abs(dx)} ${Math.abs(dy)}`}>
                ...
                <Rect x="0" y={Math.abs(dy) / 2 - beamWidth * 2} width={Math.abs(dx)} height={beamWidth * 4} fill="#e74c3c" opacity={glowOpacity * 0.5} />
                {/* Middle glow */}
                <Rect x="0" y={Math.abs(dy) / 2 - beamWidth * 1.5} width={Math.abs(dx)} height={beamWidth * 3} fill="#f39c12" opacity={glowOpacity} />
                {/* Core beam - brightest part */}
                <Rect x="0" y={Math.abs(dy) / 2 - beamWidth} width={Math.abs(dx)} height={beamWidth * 2} fill="#fff" opacity={coreOpacity * 0.8} />
                {/* Hot center - white hot at high intensity */}
                <Rect x="0" y={Math.abs(dy) / 2 - beamWidth * 0.3} width={Math.abs(dx)} height={beamWidth * 0.6} fill="#fff5cc" opacity={coreOpacity * intensity} />
                {/* Animated flames along beam */}
                {Array.from({
              length: Math.floor(3 + intensity * 5)
            }).map((_, i) => <Circle key={i} cx={Math.abs(dx) * (0.2 + i * 0.15 % 0.8)} cy={Math.abs(dy) / 2} r={beamWidth * (0.5 + Math.sin(Date.now() / 100 + i) * 0.3)} fill="#f39c12" opacity={0.6 + Math.sin(Date.now() / 80 + i * 2) * 0.3} />)}
              </Svg>
            </View>;
      }
      if (effect.type === 'electro_aura') {
        ...
        return <View key={effect.id} style={{
          ...
          zIndex: 5
              ...
              <Svg width={effect.radius * 2} height={effect.radius * 2} viewBox={`0 0 ${effect.radius * 2} ${effect.radius * 2}`}>
                {/* Outer glow circle */}
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * 0.95} fill="none" stroke="#3498db" strokeWidth="2" strokeDasharray="5 5" opacity={0.4} />
                {/* Inner fill - transparent blue */}
                <Circle cx={effect.radius} cy={effect.radius} r={effect.radius * 0.9} fill="#3498db" opacity={0.15} />
                {/* Electric particles */}
...
const Projectile = ({
  ...
  if (type === 'arrow' || type === 'spear') {
    return <View style={[styles.arrowContainer, {
      ...
      transform: [{
        ...
        <View style={[styles.arrowShaft, type === 'spear' && {
        backgroundColor: '#2ecc71',
        height: 3
      }]} />
        <View style={[styles.arrowHead, type === 'spear' && {
  ...
  if (type === 'witch_projectile') {
    ...
    return <View style={[styles.witchProjectile, {
      left: position.x - 4,
      top: position.y - 4
    }]} />;
  }
  if (type === 'dragon_fire') {
    ...
    return <View style={[styles.dragonFire, {
      left: position.x - 6,
      top: position.y - 6
    }]} />;
  }
  if (type === 'fireball_small') {
    return <View style={[styles.fireballSmall, {
      left: position.x,
      top: position.y
    }]} />;
  }
  ...
  if (type === 'fireball_spell') {
    return <View style={[styles.fireballSpell, {
      left: position.x,
      top: position.y
    }]} />;
  }
  ...
  if (type === 'zap_spell') {
    ...
    return <View style={[styles.zapSpell, {
      left: position.x - 25,
      top: position.y - 25
    }]}>
  ...
  if (type === 'arrows_spell') {
    ...
    return <View style={[styles.arrowsSpell, {
      left: position.x - 20,
      top: position.y - 20
    }]}>
  ...
  if (type === 'poison_spell' || type === 'rage_spell') {
    ...
    return <View style={{
      position: 'absolute',
      left: position.x - spellRadius,
      top: position.y - spellRadius,
      width: diameter,
      height: diameter
    }}>
    ...
    return <View style={{
      ...
      height: Math.abs(position.targetY - position.y) + 10
        ...
        <Svg width="100%" height="100%" viewBox={`0 0 ${Math.abs(position.targetX - position.x) + 10} ${Math.abs(position.targetY - position.y) + 10}`}>
          <Path d={`M${position.x > position.targetX ? 0 : Math.abs(position.targetX - position.x) + 10} 0 L${position.x > position.targetX ? Math.abs(position.targetX - position.x) + 10 : 0} ${Math.abs(position.targetY - position.y)}`} stroke="#00BFFF" strokeWidth="3" fill="none" opacity="0.8" />
    ...
    return <View style={{
      ...
      height: boltHeight
        ...
        <Svg width="100%" height="100%" viewBox={`0 0 ${boltWidth} ${boltHeight}`}>
          ...
          <Path d={pathD} stroke="#FFFFFF" strokeWidth="4" fill="none" opacity="1" strokeLinecap="round" strokeLinejoin="round" />
          {/* Outer glow - blue */}
          <Path d={pathD} stroke="#00BFFF" strokeWidth="8" fill="none" opacity="0.5" strokeLinecap="round" />
          {/* Electric sparks branching off */}
          <Path d={`M${boltWidth / 2} ${boltHeight / 2} L${boltWidth / 2 + 15} ${boltHeight / 2 - 10}`} stroke="#FFFF00" strokeWidth="2" fill="none" opacity="0.8" />
          <Path d={`M${boltWidth / 2 + 15} ${boltHeight / 2 - 10} L${boltWidth / 2 + 20} ${boltHeight / 2 - 15}`} stroke="#FFFF00" strokeWidth="2" fill="none" opacity="0.6" />
          <Path d={`M${boltWidth / 2 + 15} ${boltHeight / 2 - 10} L${boltWidth / 2 + 18} ${boltHeight / 2 - 5}`} stroke="#FFFF00" strokeWidth="2" fill="none" opacity="0.6" />
    ...
    return <View style={{
      ...
      height: 60
        ...
        <Svg width="60" height="60" viewBox="0 0 60 60">
          ...
          <Path d="M30 0 L25 25 L35 25 L20 40 L30 40 L30 60" stroke="#f1c40f" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
          {/* Glow */}
          <Path d="M30 0 L25 25 L35 25 L20 40 L30 40 L30 60" stroke="#fff" strokeWidth="6" fill="none" opacity="0.5" strokeLinecap="round" strokeLinejoin="round" />
          {/* Impact flash */}
  ...
  return <View style={[styles.cannonball, {
    left: position.x,
    top: position.y
  }]} />;
};
const Unit = ({
  ...
  useEffect(() => {
    if (unit.kamikaze && !unit.isJumpingAttack) {
      Animated.loop(Animated.sequence([Animated.timing(bounceAnim, {
        toValue: -8,
        // Jump up
        duration: 300,
        useNativeDriver: true
  ...
  return <View style={[styles.unit, {
    left: unit.x - unitSize / 2,
    top: unit.y - unitSize / 2,
    width: unitSize,
    height: unitSize
  }, isStealthed && {
    opacity: 0.4
      ...
      {Boolean(unit.range && unit.range > 0 && (unit.type === 'building' || unit.spawnDamage)) && <View style={{
      position: 'absolute',
      ...
      {isSpawning && <Animated.View style={{
      position: 'absolute',
      top: -8,
      left: -8,
      right: -8,
      ...
      {unit.stunUntil > Date.now() && <View style={{
      position: 'absolute',
      top: -4,
      left: -4,
      right: -4,
      ...
      zIndex: 6
        ...
        fontSize: 12
          ...
          {/* Electric crackles */}
          <Svg width={unitSize + 8} height={unitSize + 8} viewBox="0 0 40 40" style={{
        position: 'absolute'
      }}>
      ...
      zIndex: 10
        ...
        {unit.currentShieldHp > 0 && <View style={{
        width: '100%',
        height: 4,
        backgroundColor: '#333',
        borderRadius: 2,
        ...
        overflow: 'hidden'
            ...
            <View style={{
          width: `${unit.currentShieldHp / unit.shieldHp * 100}%`,
          height: '100%',
...
const LobbyHeader = () => <View style={styles.lobbyHeader}>
...
const ChestSlots = ({
  ...
  return <View style={styles.chestSlotsContainer}>
      <Text style={styles.chestSlotsTitle}>CHESTS</Text>
      <View style={styles.chestRow}>
        {[0, 1, 2, 3].map(index => {
        const chest = chests.find(c => c.slotIndex === index);
        if (!chest) {
          return <View key={index} style={styles.chestSlotEmpty}>
        ...
        return <TouchableOpacity key={index} style={[styles.chestSlot, {
          borderColor: getChestColor(chest.type),
          borderWidth: 2
        }]} onPress={() => isUnlocked ? onOpen(chest) : onUnlock(chest)} disabled={isUnlocking}>
              <Text style={[styles.chestText, {
            color: getChestColor(chest.type)
          }]}>{chest.type}</Text>
              {isUnlocked ? <Text style={styles.chestOpenText}>OPEN!</Text> : isUnlocking ? <Text style={styles.chestTimer}>{formatChestTime(chest.timeLeft)}</Text> : <Text style={styles.chestLockedText}>{formatChestTime(chest.unlockTime)}</Text>}
              {isUnlocking && <Text style={{
            fontSize: 10,
            color: '#f1c40f'
          }}>Unlocking...</Text>}
            </TouchableOpacity>;
...
const ShopTab = () => {
  ...
  return <ScrollView style={styles.shopContainer} contentContainerStyle={{
    paddingBottom: 100
  }}>
      ...
      <View style={styles.dealsGrid}>
        {dailyDeals.map((deal, index) => <View key={index} style={styles.dealCard}>
            <View style={[styles.dealRarityBar, {
          backgroundColor: RARITY_COLORS[deal.rarity]
        }]} />
...
const EventsTab = () => <ScrollView style={styles.eventsContainer} contentContainerStyle={{
  paddingBottom: 100
}}>
...
const ElixirDroplet = ({
  ...
  alignItems: 'center'
    ...
    position: 'absolute'
      ...
      <Path d="M50 5 Q85 45 85 70 A35 35 0 1 1 15 70 Q15 45 50 5 Z" fill={isDouble ? '#FFD700' : '#D442F5'} stroke="white" strokeWidth="4" />
    </Svg>
    {value !== undefined && <Text style={{
    color: 'white',
    fontWeight: 'bold',
    fontSize: size * 0.5,
    textShadowColor: 'black',
...
const DeckStats = ({
  ...
  return <View style={styles.deckStatsContainer}>
...
const CollectionCard = memo(({
  ...
  return <View ref={componentRef} style={{
    opacity: isDragging ? 0.3 : 1
  }} {...panResponder.panHandlers}>
      <TouchableOpacity style={[styles.deckCard, !isLegendary && {
      ...
      opacity: isInDeck ? 0.5 : 1
    }]} onPress={() => onTap(card)} delayLongPress={200} activeOpacity={0.7}>
        {isLegendary && <Svg width="70" height="85" viewBox="0 0 60 75" style={{
        position: 'absolute',
        top: 0,
        left: 0
      }}>
            ...
            <Polygon points="30,2 58,18 58,57 30,73 2,57 2,18" fill="rgba(255, 255, 255, 0.95)" stroke="url(#rainbow_collection)" strokeWidth="2" />
          </Svg>}
        ...
        {isInDeck && <View style={styles.deckCardBadge}>
            <Text style={styles.deckCardBadgeText}>✓</Text>
          </View>}
      </TouchableOpacity>
    </View>;
}, (prev, next) => {
  return prev.card.id === next.card.id && prev.isInDeck === next.isInDeck && prev.isDragging === next.isDragging;
});
...
const CardMenu = memo(({
  ...
  if (!card) return null;
  return <Modal animationType="fade" transparent={true} visible={!!card} onRequestClose={onClose}>
      <TouchableOpacity style={styles.cardMenuOverlay} activeOpacity={1} onPress={onClose}>
        <View style={styles.cardMenuContent}>
          ...
          <View style={styles.cardMenuButtons}>
            <TouchableOpacity style={[styles.cardMenuButton, styles.cardMenuButtonInfo]} onPress={() => onInfo(card)}>
              <Text style={styles.cardMenuButtonText}>📊 Info</Text>
            </TouchableOpacity>

            <Text style={styles.cardMenuOr}>OR</Text>

            <TouchableOpacity style={[styles.cardMenuButton, styles.cardMenuButtonSwap]} onPress={() => onSwap(card)}>
...
const DeckSlotSelector = memo(({
  ...
  return <Modal animationType="slide" transparent={true} visible={!!visible} onRequestClose={onClose}>
      <TouchableOpacity style={styles.cardMenuOverlay} activeOpacity={1} onPress={onClose}>
        <View style={styles.slotSelectorContent}>
          ...
          <View style={styles.slotSelectorDeck}>
            <View style={styles.slotSelectorSlotRow}>
              {(cards || []).slice(0, 4).map((card, index) => <TouchableOpacity key={card.id} style={styles.slotSelectorSlot} onPress={() => onSwap(index)}>
            ...
            <View style={styles.slotSelectorSlotRow}>
              {(cards || []).slice(4, 8).map((card, index) => <TouchableOpacity key={card.id} style={styles.slotSelectorSlot} onPress={() => onSwap(index + 4)}>
                  <UnitSprite id={card.id} isOpponent={false} size={45} />
                  <Text style={styles.slotSelectorSlotName}>{card.name || 'Card'}</Text>
                  <View style={styles.slotSelectorSlotCost}>
                    <Text style={styles.slotSelectorSlotCostText}>{card.cost || 0}</Text>
          ...
          <TouchableOpacity style={styles.slotSelectorCancel} onPress={onClose}>
...
const DeckTab = ({
  ...
  const handleDragEnd = useCallback(gesture => {
    const target = dropZones.current.find(zone => gesture.moveX >= zone.x && gesture.moveX <= zone.x + zone.width && gesture.moveY >= zone.y && gesture.moveY <= zone.y + zone.height);
    if (target) {
  ...
  const renderCollectionCard = useCallback(({
    ...
    margin: 3
      ...
      <CollectionCard card={item} isInDeck={cards.some(c => c?.id === item.id)} isDragging={localDraggingCard?.id === item.id} onTap={handleCollectionCardTap} onDragStart={handleDragStart} onDragMove={g => dragHandlers?.move?.(g.moveX, g.moveY)} onDragEnd={handleDragEnd} globalDragHandlers={dragHandlers} />
    </View>, [cards, localDraggingCard, handleCollectionCardTap, handleDragStart, handleDragEnd, dragHandlers]);
  return <View style={styles.deckTabContainer}>
      ...
      <View style={styles.deckGridContainer}>
        <View style={styles.deckHeaderRow}>
          ...
          <View style={styles.deckSelectorMini}>
            {allDecks.map((_, i) => <TouchableOpacity key={i} onPress={() => setSelectedDeckIndex(i)} style={[styles.deckDot, selectedDeckIndex === i && styles.deckDotActive]} />)}
          </View>
        </View>

        <View style={styles.deckCardGrid}>
          <View style={styles.cardRowCompact}>
            {cards.slice(0, 4).map((card, i) => <TouchableOpacity key={card.id} ref={el => deckSlotRefs.current[i] = el} onPress={() => handleDeckCardTap(card)} style={[styles.deckCardCompact, {
          ...
          <View style={styles.cardRowCompact}>
            {cards.slice(4, 8).map((card, i) => <TouchableOpacity key={card.id} ref={el => deckSlotRefs.current[i + 4] = el} onPress={() => handleDeckCardTap(card)} style={[styles.deckCardCompact, {
            borderColor: RARITY_COLORS[card.rarity]
          }]}>
                <UnitSprite id={card.id} size={35} />
                <View style={styles.cardCostSmall}><Text style={styles.cardCostSmallText}>{card.cost}</Text></View>
      ...
      <FlatList data={filteredCards} keyExtractor={item => item.id} numColumns={4} renderItem={renderCollectionCard} scrollEnabled={scrollEnabled} contentContainerStyle={{
      paddingBottom: 100
    }} columnWrapperStyle={{
      ...
      <Modal animationType="fade" transparent visible={!!selectedCard} onRequestClose={() => setSelectedCard(null)}>
        <View style={styles.cardDetailModal}>
          ...
          borderColor: RARITY_COLORS[selectedCard?.rarity] || '#F1C40F'
        }]}>
    ...
    <View style={styles.crownChestBar}>
      ...
      <View style={styles.crownProgressContainer}>
        <View style={styles.crownProgressBar}>
          <View style={[styles.crownProgressFill, {
          width: '60%'
        }]} />
        </View>
    ...
    <View style={styles.arenaMainView}>
      ...
      <View style={styles.trophyRoadContainer}>
        <View style={styles.trorophyRoadTrack}>
          <View style={[styles.trophyRoadFill, {
          width: '75%'
        }]} />
          <View style={[styles.trophyMarker, {
          left: '75%'
        }]}>
...
const SocialTab = () => {
  const [messages, setMessages] = useState([{
    ...
    id: '2',
    user: 'PrincessLover',
    text: 'Can someone donate Wizards?',
    role: 'Member',
    time: '1h ago'
  }, {
    id: '3',
    user: 'System',
    text: 'Trainer Cheddar joined the clan.',
    role: 'System',
    time: '30m ago'
  }]);
  ...
  const renderMessage = ({
    ...
    return <View style={[styles.chatRow, isMe ? styles.chatRowMe : styles.chatRowOther]}>
        <View style={[styles.chatBubble, isMe ? styles.chatBubbleMe : styles.chatBubbleOther]}>
          {!isMe && <Text style={[styles.chatUser, {
          color: item.role === 'Elder' ? '#f1c40f' : '#3498db'
        }]}>{item.user}</Text>}
  ...
  return <View style={styles.clanTabContainer}>
      <View style={styles.socialHeader}>
        <View style={styles.clanMainInfo}>
          <View style={styles.clanBadgeLarge}><Text style={{
            fontSize: 24
          }}>🛡️</Text></View>
          <View>
        ...
        <TouchableOpacity style={styles.clanSettingsIcon}><Text style={{
          fontSize: 18
        }}>ℹ️</Text></TouchableOpacity>
      </View>
      ...
      <KeyboardAvoidingView behavior="padding" style={styles.chatContainer} keyboardVerticalOffset={80}>
        <FlatList data={messages} renderItem={renderMessage} keyExtractor={item => item.id} contentContainerStyle={{
        padding: 10
      }} />
...
const BottomNavigation = ({
  ...
  return <View style={styles.bottomNavigation}>
      {tabs.map(tab => <TouchableOpacity key={tab.id} style={[styles.tabButton, activeTab === tab.id && styles.tabButtonActive]} onPress={() => onTabChange(tab.id)}>
          <Text style={[styles.tabIcon, activeTab === tab.id && {
        transform: [{
          scale: 1.2
        }]
      }]}>{tab.icon}</Text>
...
const MainLobby = ({
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 0:
        return <ShopTab />;
      case 1:
        return <DeckTab cards={currentDeck} onSwapCards={onSwapCards} dragHandlers={dragHandlers} selectedDeckIndex={selectedDeckIndex} setSelectedDeckIndex={setSelectedDeckIndex} allDecks={allDecks} />;
      case 2:
        return <BattleTab currentDeck={currentDeck} onStartBattle={onStartGame} chests={chests} onUnlockChest={onUnlockChest} onOpenChest={onOpenChest} />;
      case 3:
        return <SocialTab />;
      case 4:
        return <EventsTab />;
  ...
  return <ImageBackground source={require('./lobby-bg.jpg')} style={styles.lobbyContainer}>
...
const GameBoard = ({
  ...
  useEffect(() => {
    if (screenShake) {
      ...
      for (let i = 0; i < shakes; i++) {
        animations.push(Animated.sequence([Animated.timing(shakeAnim, {
  ...
  return <View style={styles.container}>
      <Animated.View style={[styles.gameBoard, screenShake && {
      transform: [{
        ...
        <Modal transparent={true} visible={showSettings} animationType="fade" onRequestClose={() => setShowSettings(false)}>
          <View style={{
          flex: 1,
          ...
          alignItems: 'center'
            ...
            elevation: 5
              ...
              marginBottom: 10
                ...
                fontWeight: 'bold',
                fontSize: 16
              }}>Audio: {audioEnabled ? 'ON' : 'OFF'}</Text>
              </TouchableOpacity>
        ...
        return <View key={tower.id} style={[styles.towerContainer, styleObj]}>
      ...
      {/* Double Elixir Alert */}
      {showDoubleElixirAlert && <View style={styles.doubleElixirAlert}>
      ...
      <View style={styles.footerContainer}>
        <View style={styles.deckContainer}>
          ...
          <View style={styles.handContainer}>
            {(hand || []).map((card, index) => <Card key={`${card.id}-${index}`} card={card} isNext={false} canAfford={elixir >= card.cost} onDragStart={handleDragStart} onDragMove={handleDragMove} onDragEnd={handleDragEnd} isDragging={draggingCard && draggingCard.id === card.id} />)}
          </View>
        </View>
        ...
        <View style={styles.elixirSection}>
          <View style={styles.elixirContainer}>
            ...
            <View style={styles.elixirBarBack}>
              <View style={[styles.elixirBarFill, isDoubleElixir && styles.elixirBarFillDouble, {
              width: `${elixir / 10 * 100}%`
            }]} />
              {[...Array(9)].map((_, i) => <View key={i} style={[styles.elixirTick, {
              left: `${(i + 1) * 10}%`
            }]} />)}
            </View>
          </View>
          {/* Hidden debug button for testing */}
          <TouchableOpacity style={[styles.debugBtnSmall, {
          opacity: 0
        }]} onPress={spawnTestEnemy}>
            <Text style={{
            color: '#fff',
            fontSize: 10
          }}>Enemy</Text>
      ...
      elevation: 100
          ...
          {Boolean(draggingCard.radius || draggingCard.range && (draggingCard.type === 'building' || draggingCard.spawnDamage)) && <View style={{
        position: 'absolute',
        ...
        borderStyle: draggingCard.type === 'spell' ? 'solid' : 'dashed'
          ...
          {Boolean(draggingCard.spawnDamage) && <View style={{
        position: 'absolute',
        left: -50,
        top: -50,
        width: 100,
...
const ChestOpeningModal = ({
  ...
  const generateRewards = () => {
    ...
    const newRewards = [];
    if (gold > 0) newRewards.push({
      type: 'GOLD',
      value: gold,
      icon: '💰',
      label: 'Gold'
    });
    if (gems > 0) newRewards.push({
      type: 'GEM',
      value: gems,
      icon: '💎',
      label: 'Gems'
  ...
  return <TouchableOpacity style={styles.chestModalOverlay} activeOpacity={1} onPress={handleTap}>
      <View style={styles.chestModalContent}>
        <Text style={styles.chestModalTitle}>{chest.type} CHEST</Text>
        
        {step === 'CLOSED' && <View style={{
        alignItems: 'center'
      }}>
            <Animated.View style={[styles.chestVisual, {
          borderColor: getChestColor(chest.type),
          transform: [{
            translateX: shakeAnim
          }]
        ...
        {(step === 'OPENING' || step === 'FINISHED') && currentReward && <View style={{
        alignItems: 'center',
        width: '100%'
      }}>
            {/* Authentic CR Item Count Indicator */}
            {step === 'OPENING' && remainingCount > 0 && <View style={styles.itemCountBadge}>
            ...
            <Animated.View style={styles.rewardRevealContainer}>
              <View style={styles.rewardItemLarge}>
                {currentReward.type === 'CARD' ? <View style={{
              alignItems: 'center'
            }}>
            ...
            {step === 'FINISHED' && <TouchableOpacity style={styles.closeChestButton} onPress={onClose}>
                <Text style={styles.closeChestButtonText}>COLLECT</Text>
              </TouchableOpacity>}
            
            {step === 'OPENING' && <Text style={styles.chestTapText}>Tap to reveal next!</Text>}
          </View>}
      </View>
    </TouchableOpacity>;
};
...
const FriendlyBattleModal = ({
  ...
  return <Modal visible={visible} transparent={true} animationType="slide">
      <TouchableOpacity style={styles.chestModalOverlay} activeOpacity={1} onPress={onClose}>
        <View style={styles.friendlyModalContent} onStartShouldSetResponder={() => true}>
          ...
          {mode === 'MENU' && <View style={styles.roomButtonsRow}>
          ...
          {mode === 'JOIN' && <View style={{
          alignItems: 'center',
          width: '100%'
        }}>
              <TextInput style={styles.roomInput} placeholder="Enter Room Code" keyboardType="numeric" maxLength={4} value={joinCode} onChangeText={setJoinCode} />
...
export default function App() {
  ...
  const [allDecks, setAllDecks] = useState([getDeckByIds(['hog_rider', 'musketeer', 'ice_golem', 'ice_spirit', 'skeletons', 'fireball', 'zap', 'cannon']),
  // Hog 2.6
  getDeckByIds(['goblin_barrel', 'princess', 'knight', 'dart_goblin', 'inferno_tower', 'rocket', 'arrows', 'skeletons']),
  // Log Bait
  getDeckByIds(['pekka', 'bandit', 'battle_ram', 'electro_wizard', 'magic_archer', 'zap', 'poison', 'royal_ghost']),
  // Pekka Bridge Spam
  getDeckByIds(['golem', 'night_witch', 'baby_dragon', 'mega_minion', 'lightning', 'zap', 'elite_barbarians', 'mini_pekka']),
  // Golem Beatdown
  ...
  const [globalDragPosition, setGlobalDragPosition] = useState({
    x: 0,
    y: 0
  });
  ...
  useEffect(() => {
    enemyHandRef.current = enemyHand;
  }, [enemyHand]);
  useEffect(() => {
    enemyNextCardRef.current = enemyNextCard;
  }, [enemyNextCard]);
  useEffect(() => {
    enemyDeckQueueRef.current = enemyDeckQueue;
  }, [enemyDeckQueue]);
  useEffect(() => {
    lastPlayedCardRef.current = lastPlayedCard;
  }, [lastPlayedCard]);
  useEffect(() => {
    enemyLastPlayedCardRef.current = enemyLastPlayedCard;
  }, [enemyLastPlayedCard]);
  const concedeGame = () => {
    setGameOver('LOSE');
  };
    ...
    setTargetElixir(currentElixir => {
      ...
      if (actualCard.type === 'spell') {
        ...
        if (actualCard.id === 'lightning') {
          ...
          spellSpeed = 100;
          const allTargets = [...(unitsRef.current || []).filter(u => u.isOpponent !== isOpponent), ...(towersRef.current || []).filter(t => t.isOpponent !== isOpponent && t.hp > 0)];
          ...
          setProjectiles(prev => [...prev, {
            ...
            spawnCount: actualCard.spawnCount || 3,
            isOpponent
          }]);
        ...
        for (let i = 0; i < count; i++) {
          ...
          newUnits.push({
            ...
            lastDamageRampTime: Date.now(),
            lastTargetId: null,
            bombDrops: actualCard.bombDrops || false,
            lastBombDrop: Date.now(),
            shotgunSpread: actualCard.shotgunSpread || false,
      ...
      if (!isOpponent) setLastPlayedCard(actualCard);else setEnemyLastPlayedCard(actualCard);
  ...
  const spawnTestEnemy = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      newUnits.push({
        ...
        jumps: stats.jumps || false,
        // Hog Rider can jump over river
        spawns: stats.spawns,
        ...
        lifetimeDuration: stats.lifetime,
        // Store lifetime duration in seconds
        spawnTime: Date.now(),
        // Track when building was spawned for HP depreciation
        maxHp: stats.hp,
        // Store initial max HP for depreciation calculation
        stunUntil: 0,
  ...
  const checkWinner = () => {
    ...
    let result = 'DRAW';
    if (playerTowers > opponentTowers) result = 'VICTORY';else if (opponentTowers > playerTowers) result = 'DEFEAT';
    setGameOver(result);
  ...
  useEffect(() => {
    ...
    const loop = setInterval(() => {
      ...
      let currentUnits = (unitsRef.current || []).map(u => {
        ...
        if (u.isJumpingAttack) {
          ...
          if (progress >= 1) {
            ...
            const target = (unitsRef.current || []).find(t => t.id === u.jumpTargetId) || (nextTowers || []).find(t => t.id === u.jumpTargetId);
        ...
        if (u.hidden) {
          if (u.spriteId === 'tesla') {
            ...
            const hasEnemyInRange = (unitsRef.current || []).some(enemy => enemy.isOpponent !== u.isOpponent && enemy.hp > 0 && Math.sqrt(Math.pow(enemy.x - u.x, 2) + Math.pow(enemy.y - u.y, 2)) <= detectionRange);
        ...
        let targets = (nextTowers || []).filter(t => t.isOpponent !== u.isOpponent && t.hp > 0 && t.type === 'princess');
        ...
        if (u.targetType !== 'buildings') {
          ...
          const unitTargets = (unitsRef.current || []).filter(targetUnit => targetUnit.isOpponent !== u.isOpponent && targetUnit.hp > 0 && !targetUnit.hidden?.active &&
          // Untargetable if hidden (Royal Ghost, Tesla)
          !targetUnit.isZone && (
          // Cannot target zones (graveyard, etc.)
        ...
        if (closestTarget && minDist <= actualRange + 25) {
                ...
                if (u.shotgunSpread) {
                  ...
                  for (let i = -4; i <= 5; i++) {
                    ...
                    nextProjectiles.push({
                      ...
                      y: u.y,
                      targetId: null,
                      // Shotgun bullets are location based
                      targetX: tX,
                      targetY: tY,
                      speed: 15,
                      damage: Math.floor(damageToDeal / 10),
                      // Divide total damage by pellet count
                      type: 'bullet',
                      // pellets
                      attackerId: u.id,
                      isOpponent: u.isOpponent
                  ...
                  for (let i = 0; i < pelletCount; i++) {
                    ...
                    nextProjectiles.push({
                      ...
                      y: u.y,
                      targetId: null,
                      // Firecracker rockets are location based
                      targetX: tX,
                      targetY: tY,
                      speed: 12,
                      damage: Math.floor(damageToDeal / pelletCount),
                      // Divide damage by rocket count
                      type: 'firecracker',
                      splash: true,
                      splashRadius: 25,
                      stun: u.stun || 0,
                  ...
                  return prevUnits.map(enemy => {
                    ...
                    if (enemy.isOpponent !== u.isOpponent && enemy.hp > 0) {
                      ...
                      if (dist <= shockRadius) {
                        ...
                        return {
                          ...enemy,
                          stunUntil: now + shockStunDuration
                        };
                      }
                    }
                    return enemy;
              ...
              if (u.kamikaze) {
                ...
                if (u.chain > 0) {
                  chainEvents.push({
                    ...
                    chainCount: Math.min(u.chain - 1, 3),
                    // Max 3 additional targets (4 total)
                    damage: damageToDeal,
                    stun: u.stun || 0,
                    isOpponent: u.isOpponent,
                    startX: u.x,
          ...
          if (isJumpingNow) {
              ...
              if (u.spriteId === 'skeleton_barrel') {
                ...
                if (distToTarget < 30) {
                  return {
                    ...
                    y: nextY,
                    hp: 0
                  }; // Die immediately to trigger death spawn
                }
              }
            } else {
            ...
            const lanePrincess = (nextTowers || []).find(t => t.type === 'princess' && t.isOpponent !== u.isOpponent && (u.lane === 'LEFT' && t.x < width / 2 || u.lane === 'RIGHT' && t.x > width / 2));
        ...
        unitsThatDied.forEach(deadUnit => {
          ...
          if (deadUnit.deathRage && deadUnit.spriteId === 'lumberjack') {
            ...
            setProjectiles(prev => [...prev, {
              ...
              damage: 0,
              radius: 50,
              // Rage spell radius (2.5 tiles = 50 pixels)
              type: 'rage_spell',
              isSpell: true,
              isRage: true,
              hit: true,
            ...
            if (spawnCard) {
              ...
              for (let i = 0; i < deathSpawnCount; i++) {
                ...
                unitsToSpawn.push({
                  ...
                  lane: deadUnit.lane,
                  lastAttack: 0,
                  // Ready to attack immediately (or add small delay)
                  spriteId: spawnCard.id,
                  type: spawnCard.type,
                  range: spawnCard.range,
                  damage: spawnCard.damage,
          ...
          if (deadUnit.deathDamage || deadUnit.spriteId === 'ice_golem') {
            // Check card definition for deathDamage
            ...
            if (cardDef && (cardDef.deathDamage || cardDef.deathSlow)) {
              if (cardDef.deathBombDelay) {
                ...
                setVisualEffects(prev => [...prev, {
                  id: Date.now() + Math.random(),
                  type: 'bomb_drop',
                  // We can add this visual or just use the projectile
                  x: deadUnit.x,
                  y: deadUnit.y,
                  radius: 20,
                  startTime: Date.now(),
        ...
        nextTowers = nextTowers.map(tower => {
          if (tower.isOpponent !== event.attacker.isOpponent && tower.hp > 0) {
            ...
            if (dist <= splashRadius + 20) {
              ...
              return {
                ...tower,
                hp: tower.hp - Math.floor(event.damage * 0.5)
              };
            }
          }
          return tower;
        ...
        currentUnits = currentUnits.map(unit => {
          if (unit.isOpponent === event.isOpponent && unit.hp > 0) {
            ...
            if (dist <= event.radius) {
              return {
                ...unit,
                hp: Math.min(unit.maxHp || unit.hp + event.amount, unit.hp + event.amount)
              };
            }
          }
          return unit;
        ...
        nextTowers = nextTowers.map(tower => {
          if (tower.isOpponent === event.isOpponent && tower.hp > 0) {
            ...
            if (dist <= event.radius) {
              return {
                ...tower,
                hp: Math.min(tower.maxHp || tower.hp + event.amount, tower.hp + event.amount)
              };
            }
          }
          return tower;
      ...
      let activeProjectiles = nextProjectiles.map(p => {
        ...
        if (p.type === 'tesla_lightning') {
          return {
            ...
            targetY,
            hit: !p.damageDealt,
            // Only hit if not yet dealt damage
            keepVisual: true
          };
        }
        ...
        if (p.pierce) {
          ...
          const targets = [...(unitsRef.current || []).filter(u => u.isOpponent !== p.isOpponent && u.hp > 0), ...(nextTowers || []).filter(t => t.isOpponent !== p.isOpponent && t.hp > 0)];
          targets.forEach(t => {
      ...
      if (hits.length > 0) {
        hits.forEach(h => {
          ...
          if (h.isSpell) {
            ...
            if (h.chain > 0) {
              const target = (unitsRef.current || []).find(u => u.id === h.targetId) || (nextTowers || []).find(t => t.id === h.targetId);
              if (target) {
                chainEvents.push({
                  attackerId: h.attackerId,
                  primaryTarget: target,
                  chainCount: Math.min(h.chain - 1, 3),
                  // Max 3 more
                  damage: h.damage,
                  stun: h.stun || 0,
                ...
                if (u.id !== h.targetId && u.hp > 0) {
                  ...
                  if (isEnemy) {
                    ...
                    if (dist <= splashRadius) {
                      let updatedUnit = {
                        ...u,
                        hp: u.hp - Math.floor(h.damage * 0.5)
                      };
      ...
      activeProjectiles = activeProjectiles.filter(p => p.x > -50 && p.x < width + 50 && p.y > -50 && p.y < height + 50);
      nextTowers = nextTowers.map(tower => {
  ...
  if (!inGame && inLobby) {
    return <>
        <MainLobby activeTab={activeTab} onTabChange={setActiveTab} onStartGame={handleStartBattle} currentDeck={userCards} onSwapCards={handleSwapCards} dragHandlers={{
        start: onGlobalDragStart,
        move: onGlobalDragMove,
        end: onGlobalDragEnd
      }} selectedDeckIndex={selectedDeckIndex} setSelectedDeckIndex={setSelectedDeckIndex} allDecks={allDecks} chests={chests} onUnlockChest={handleUnlockChest} onOpenChest={handleOpenChest} onFriendlyBattle={handleFriendlyBattle} />
        {openingChest && <ChestOpeningModal chest={openingChest} onClose={handleCollectRewards} />}
        <FriendlyBattleModal visible={friendlyModalVisible} onClose={() => setFriendlyModalVisible(false)} socket={socketRef.current} />
      </>;
  }
  return <>
          <GameBoard towers={towers} units={units} projectiles={projectiles} visualEffects={visualEffects} setVisualEffects={setVisualEffects} screenShake={screenShake} setScreenShake={setScreenShake} timeLeft={timeLeft} gameOver={gameOver} elixir={elixir} enemyElixir={enemyElixir} hand={hand} nextCard={nextCard} draggingCard={draggingCard} dragPosition={dragPosition} handleDragStart={handleDragStart} handleDragMove={handleDragMove} handleDragEnd={handleDragEnd} spawnTestEnemy={spawnTestEnemy} formatTime={formatTime} onRestart={dest => resetGame(dest)} score={score} isDoubleElixir={isDoubleElixir} showDoubleElixirAlert={showDoubleElixirAlert} audioEnabled={audioEnabled} setAudioEnabled={setAudioEnabled} onConcede={concedeGame} />      {globalDraggingCard && <View style={[styles.dragProxy, {
      position: 'absolute',
...
const styles = StyleSheet.create({
  ...
  menuOverlay: {
    flex: 1,
    backgroundColor: 'rgba(0, 0, 0, 0.3)',
    // Lighter overlay for better bg visibility
    width: '100%',
    alignItems: 'center',
    paddingVertical: 30
  },
  ...
  bridge: {
    ...
    width: 60,
    height: 54,
    // Slightly longer than river to overlap
    top: -2,
